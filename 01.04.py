...
async def add_comment_to_issue(
    ...
    """Add a comment to an existing issue in OKDesk."""

    # First, validate that the contact has access to this issue
    ...
    comment_data = {
        ...
        "public": True,
        "author_id": contact_id,  # Using contact_id as author_id
        "author_type": "contact"
    }
    
    params = {"api_token": OKDESK_API_TOKEN}
    
    logger.info(f"Adding comment to issue ID {issue_id} with contact ID {contact_id}")
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            data = response.json()
            logger.info(f"Comment creation response: {data}")
...
async def main():
    ...
    async def handle_issue_selection(message: types.Message, state: FSMContext):
        ...
        await message.answer(
            "✍️ Tanlangan muammoga qanday izoh qo'shmoqchisiz? Yozib yuboring:",
            reply_markup=ReplyKeyboardRemove()
        )
    ...
    async def submit_issue_comment(message: types.Message, state: FSMContext):
        """
        Submit the comment to OKDesk and confirm with the user
        """
        user_data = await state.get_data()
